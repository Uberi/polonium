...
setTimeout(function() {
doc.find("._2pt").append("<a href=\"#\" id=\"_custom_eggplant_button\" onmouseup=\"var textarea = document.getElementsByName('message_body')[0], text = '\uD83C\uDF46'; console.log(text); var scroll = textarea.scrollTop, caret = textarea.selectionStart; var before = textarea.value.substring(0, caret), after = textarea.value.substring(textarea.selectionEnd, textarea.value.length); textarea.value = before + text + after; caret += text.length; textarea.selectionStart = caret; textarea.selectionEnd = caret; textarea.focus(); textarea.scrollTop = scroll; return false;\">\uD83C\uDF46</a>");
}, 2000);
